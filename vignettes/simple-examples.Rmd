...
X = cbind(rep(1,dim(admit_data)[1]), 
          admit_data$gre, 
          admit_data$gpa)

glm_model = glm(admit ~ gre + gpa, data = admit_data, family = 'binomial')
...
The fitted values are slightly different, but very slightly and not enough to change predictive inference. We can also do predicttion as we did before using logistic.
...
logistic_fit_binomial = logistic(cbind(rep(1,32), X$wt,X$vs,X$am), X$successes, n = X$total)
...
bmark = bench::mark(as.vector(glm(y ~ X, family = 'binomial')$coefficients), 
                    as.vector(logistic(X1,y)$coeffs))
